...
def main1():
    ...
    train_dataset = datasets.ImageFolder(root=os.path.join(image_path, "train-handcrop"),
    ...
    validate_dataset = datasets.ImageFolder(root=os.path.join(image_path, "train-handcrop"),
    ...
    optimizer = optim.Adam(params, lr=0.0003, weight_decay=1e-4)
...
def main2():
    ...
    image_path = DATA_DIR
    assert os.path.exists(image_path), "{} path does not exist.".format(image_path)
    train_dataset = datasets.ImageFolder(root=os.path.join(image_path, "train-handcrop929"),
                                         transform=data_transform["train"])
    train_num = len(train_dataset)
    ...
    validate_dataset = datasets.ImageFolder(root=os.path.join(image_path, "val-handcrop162"),
                                            transform=data_transform["val"])
    val_num = len(validate_dataset)
    validate_loader = torch.utils.data.DataLoader(validate_dataset,
                                                  batch_size=batch_size, shuffle=False,
...
def main3():
    epochs = EPOCH
    msg = 'handcrop929_BSize8_dropout'
    save_dir = os.path.join(SAVE_DIR,
                            'Resnet50' + '_' + 'Epoch' + str(epochs) + '_' + datetime.now().strftime(
                                '%Y%m%d_%H%M%S') + '_' + msg)
    if os.path.exists(save_dir):
